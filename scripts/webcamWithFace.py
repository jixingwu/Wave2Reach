...
class ImageNewClass:
    def __init__(self, centerP, size):
        self.centerP = centerP
        self.size = size

    def convert2Points(self):
        leftTop = (np.int(self.centerP[0]-self.size/2), np.int(self.centerP[1]-self.size/2))
        rightBottom = (np.int(self.centerP[0]+self.size/2), np.int(self.centerP[1]+self.size/2))
...
class OpenPose:
    ...
    def callback(self, data):
        ...
        self.imageWidht = data.height
        self.imageHeight = data.width
    ...
    def process(self):
        ...
        if datum.poseKeypoints is not None:
            ...
            self.head = (np.int((human[0,0]+human[1,0])/2), np.int((human[0,1]+human[1,1])/2))
        ...
        # self.head = (np.int(self.imageHeight/2), np.int(self.imageWidht/2))
        # print(self.head)
        if self.detected_flag == True:
            ...
            if num != 0 and dist != 0:
                ...
                # face detection and add into output image
                windowSize = 100
                windowSize_2 = np.int(windowSize * 2)
                cropImage = ImageNewClass(self.head, windowSize)
                leftTop, rightBottom = cropImage.convert2Points()
                cropImage = image[leftTop[1]:rightBottom[1], leftTop[0]:rightBottom[0], :]
                print(leftTop)
                print(rightBottom)
                # print(np.shape(output)) #[720, 1280, 3]

                addImage_center = (np.int(self.imageHeight - windowSize_2/2), self.head[1])
                addImage = ImageNewClass(addImage_center, windowSize_2)
                leftTop_, rightBottom_ = addImage.convert2Points()
                addImage = cv2.resize(cropImage, dsize=(np.int(windowSize_2), np.int(windowSize_2)))
                # cv2.imshow("cropImage", cropImage)
                # cv2.imshow("addImage", addImage)
                # cv2.waitKey()
                print("--------------")
                print(addImage_center)
                print(leftTop_)
                print(rightBottom_)
                print("--------------")
                output[leftTop_[1]:rightBottom_[1], leftTop_[0]:rightBottom_[0] , :] = addImage
                rightTop = (rightBottom[0], leftTop[1])
                rightBottom = rightBottom
                leftTop_ = leftTop_
                leftBottom_ = (leftTop_[0], rightBottom_[1])
                cv2.rectangle(output, leftTop, rightBottom, (0,0,255), 2)
                cv2.line(output, rightTop, leftTop_, (0,255,0), 2)
                cv2.line(output, rightBottom, leftBottom_, (0,255,0),2)
