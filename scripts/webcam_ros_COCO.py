...
class OpenPose:
    ...
    def process(self):
        ...
        if datum.poseKeypoints is not None:
            ...
            lx = (human[7,1] - human[0,1]) * 100 / (human[1,1] - human[0,1])
            rx = (human[4,1] - human[0,1]) * 100 / (human[1,1] - human[0,1])
            # print("lx=" + str(lx))
            # print("rx=" + str(rx))
            ...
            self.box = (np.int(human[8,0]), np.int(human[1,1]), np.int(human[11,0]), np.int(human[11,1]))
            print(self.point)
            print(self.box)
